...
# Initialize database with path in database folder
crop_db = CropMasterDB(db_path=str(db_dir / 'crop_master.db'))

print(f"✅ Database initialized at: {db_dir / 'crop_master.db'}")
...
def run_simulation_api():
    """Run simulation with enhanced interface"""
    try:
        data = request.get_json()
        print(f"\n{'='*70}")
        print(f"📊 NEW SIMULATION REQUEST RECEIVED")
        print(f"{'='*70}")
        print(f"Data: {data}")
        
        # Extract data
        crop_name = data.get('crop_name')
        model_type = data.get('model_type', 'fixed_bed')
        ...
        process_params = {
            'initial_moisture': float(data.get('initial_moisture', 0)),
            'target_moisture': float(data.get('target_moisture', 0)),
            'grain_temp': float(data.get('grain_temp', 0)),
            'air_temp': float(data.get('air_temp', 0)),
            'air_rh': float(data.get('air_rh', 0)),
            'air_flow_rate': float(data.get('air_flow_rate', 0)),
            'grain_flow_rate': float(data.get('grain_flow_rate', 0)),
            'width': float(data.get('width', 0)),
            'length': float(data.get('length', 0))
        }
        
        # Import and use enhanced interface
        from enhanced_fortran_interface import EnhancedFortranDryerInterface
        interface = EnhancedFortranDryerInterface(base_dir=str(BASE_DIR.parent))
        
        # Run simulation (this will show terminal output)

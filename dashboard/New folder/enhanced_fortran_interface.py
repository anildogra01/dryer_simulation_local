...
class EnhancedFortranDryerInterface:
    ...
    def run_simulation(self, model_type, crop_name, process_params, user_info):
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                return {'success': False, 'error': f"Simulation failed: {result.stderr}"}
    ...
    def create_input_file(self, model_type, crop_name, process_params, user_info):
        ...
        with open(input_path, 'w') as f:
            # ========================================
            # HEADER LINES (read with '(A)' format)
            # ========================================
            print(f"   📝 Writing header lines...")
            f.write(f"{user_info.get('user_name', 'N/A')}\n")
            f.write(f"{user_info.get('project_name', 'N/A')}\n")
            f.write(f"{user_info.get('address', 'N/A')}\n")
            f.write(f"{crop_name}\n")
            f.write(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            print(f"   ✅ Wrote 5 header lines")
            
            # ========================================
            # NUMERIC DATA (read with * format)
            # ========================================
            print(f"   📝 Writing process parameters...")
            f.write(f"{xmo}  {thin}  {tin}  {hin}  {cfm}\n")
            f.write(f"{depth}  {indpr_real}\n")
            f.write(f"{nlpf_real}  {tt}\n")
            f.write(f"{tbtpr}  {xmend}\n")
            print(f"   ✅ Wrote 4 process parameter lines")
            
            # ========================================
            # CROP PROPERTIES FROM DATABASE
            # ========================================
            print(f"   📝 Writing crop properties...")
            print(f"      sa = {crop['specific_surface_area']}")
            print(f"      ca = {crop['heat_capacity_air']}")
            print(f"      cp = {crop['heat_capacity_dry_product']}")
            f.write(f"{crop['specific_surface_area']}\n")
            f.write(f"{crop['heat_capacity_air'] / 1000.0}\n")  # Convert J/kg.K to BTU/lb.F
            f.write(f"{crop['heat_capacity_dry_product'] / 1000.0}\n")
            f.write(f"{crop['heat_capacity_water_vapor'] / 1000.0}\n")
            f.write(f"{crop['dry_bulk_density'] / 16.018}\n")  # Convert kg/m3 to lb/ft3
            f.write(f"{crop['latent_heat_water'] / 2326.0}\n")  # Convert J/kg to BTU/lb
            print(f"   ✅ Wrote 6 crop property lines")
        
        print(f"   ✅ Input file created: {input_path}")
        print(f"   📊 Crop properties from database: {crop_name}")
        print(f"   📊 Total lines written: 15")
        return input_path
    
    def parse_output(self, model_dir, model_type):
        """Parse FIXED.OUT file"""
        results = {}
        ...
        try:
            ...
            if len(content) > 0:
                print(f"   ✅ Output file found ({len(content)} bytes)")
                # Store raw output for now

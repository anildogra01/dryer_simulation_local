...
def index():
    ...
    try:
        ...
        template_path = os.path.join(app.template_folder, 'dashboard_with_monitoring.html')
...
def health_check():
    ...
    try:
        crops = crop_db.get_all_crops()
        sims = transaction_db.get_recent_simulations(limit=1)
        
        return jsonify({
            'status': 'healthy',
            'database': 'connected',
            'crops_count': len(crops),
            'crops': [c['crop_name'] for c in crops],

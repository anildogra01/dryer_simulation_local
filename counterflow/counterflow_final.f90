program counterflow_simple
    !-------------------------------------------------------------------
    ! COUNTERFLOW DRYER SIMULATION - Simplified (No Powell Optimization)
    !-------------------------------------------------------------------
    use sychart_package
    use emc_corn
    use rkamsub_modern
    ...
    ! Open files
    write(*,*) 'COUNTERFLOW DRYER - Opening COUNTER.DAT...'
    open(unit=5, file='COUNTER.DAT', status='old', action='read', err=999)
    open(unit=6, file='COUNTER.OUT', status='replace', action='write')
    ...
    write(*,*) 'Starting simulation (simple integration, no optimization)...'
    
    ! Print headings
    write(6, 211)
    
    ! Initialize Y array (use simple initial guess)
    y(1) = tin        ! Air temperature
    ...
    write(*,*) 'Results in COUNTER.OUT'
    write(*,*) 'Final MC =', y(2)
    
    stop
    
999 write(*,*) 'ERROR: Cannot open COUNTER.DAT'
    stop
...
210 format(1h1, 43x, 'M I C H I G A N  S T A T E  U N I V E R S I T Y', &
           ...
           /40x, 'C O U N T E R F L O W  D R Y E R  M O D E L', /59x, 'U S I N G  T H E', &
...
end program counterflow_simple


!-----------------------------------------------------------------------
! DERFUN - Derivative function
!-----------------------------------------------------------------------
subroutine derfun()
    use sychart_package
    use emc_corn
